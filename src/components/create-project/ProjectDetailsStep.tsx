...
export default function ProjectDetailsStep({
  ...
  // Track the last URL we auto-generated for, to avoid re-triggering
  const autoGeneratedForUrl = useRef<string | null>(initialData?.websiteUrl ?? null);

  // Auto-generate description when a valid URL is entered and description is empty
  useEffect(() => {
    if (
      !websiteUrl ||
      !isValidUrl(websiteUrl) ||
      isGeneratingDescription ||
      autoGeneratedForUrl.current === websiteUrl
    ) {
      return;
    }

    // Only auto-generate when description is empty or hasn't been touched
    const currentDesc = description?.trim() ?? "";
    if (currentDesc.length > 0) {
      // User already has a description, don't overwrite
      autoGeneratedForUrl.current = websiteUrl;
      return;
    }
  ...
  }, [websiteUrl, description, isGeneratingDescription, setValue]);
